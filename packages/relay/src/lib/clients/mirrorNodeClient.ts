...
export class MirrorNodeClient {
    ...
    protected createAxiosClient(
        ...
        const retryErrorCodes: Array<number> = process.env.MIRROR_NODE_RETRY_CODES ? JSON.parse(process.env.MIRROR_NODE_RETRY_CODES) : [404]; // by default we should only retry on 404 errors
        ...
        axiosRetry(axiosClient, {
            ...
            retryCondition: (error) => {
                return !error?.response?.status || retryErrorCodes.includes(error?.response?.status);
            },
