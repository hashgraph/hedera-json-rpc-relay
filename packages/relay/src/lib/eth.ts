...
export class EthImpl implements Eth {
  ...
  async getTransactionByHash(hash: string, requestId?: string) {
    ...
    return new Transaction({
      ...
      maxFeePerGas: maxFee,
      nonce: EthImpl.nullableNonceNumberTo0x(contractResult.nonce),
      r: rSig,
      s: sSig,
  ...
  static nullableNonceNumberTo0x(input: number | BigNumber): string | null {
    ...
    return this.mirrorNodeClient.getContractResultsByAddressAndTimestamp(to, timestamp, requestId)
      .then(contractResultDetails => {
        ...
        return new Transaction({
          ...
          nonce: EthImpl.nullableNonceNumberTo0x(contractResultDetails.nonce),
          r: rSig,
          s: sSig,
