  ...
  shouldPreemtivelyLimit(
    ...
    if (this.isAccountWhiteListed(originalCallerAddress)) {
      ...
      if (this.remainingBudget - totalPreemtiveTransactionFee < 0) {
        this.logger.trace(
          `${requestIdPrefix} HBAR preemtive rate limit incoming call - the total preemptive transaction fee exceeds the current remaining HBAR budget due to an excessively large callData size: totalPreemtiveTransactionFee=${totalPreemtiveTransactionFee}, remainingBudget=${this.remainingBudget}, otal=${this.total}, resetTimestamp=${this.reset}, numFileCreateTxs=${numFileCreateTxs}, numFileAppendTxs=${numFileAppendTxs}, callDataSize=${callDataSize}.`,
        );
