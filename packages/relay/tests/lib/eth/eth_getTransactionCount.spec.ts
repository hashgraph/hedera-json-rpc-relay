...
describe('@ethGetTransactionCount eth_getTransactionCount spec', async function () {
  ...
  it('should return 0x0 nonce for no block consideration with not found acoount', async () => {
    ...
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, null, requestIdPrefix);
  ...
  it('should return 0x0 nonce for block 0 consideration', async () => {
    restMock.onGet(accountPath).reply(200, mockData.account);
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, '0', requestIdPrefix);
    expect(nonce).to.exist;
    expect(nonce).to.equal(EthImpl.zeroHex);
  });
  ...
  it('should return 0x0 nonce for block 1 consideration', async () => {
    restMock.onGet(accountPath).reply(200, mockData.account);
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, '1', requestIdPrefix);
    expect(nonce).to.exist;
    expect(nonce).to.equal(EthImpl.zeroHex);
  });
  ...
  it('should return latest nonce for latest block', async () => {
    restMock.onGet(accountPath).reply(200, mockData.account);
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, EthImpl.blockLatest, requestIdPrefix);
    expect(nonce).to.exist;
    expect(nonce).to.equal(numberTo0x(mockData.account.ethereum_nonce));
  });
  ...
  it('should return latest nonce for finalized block', async () => {
    restMock.onGet(accountPath).reply(200, mockData.account);
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, EthImpl.blockFinalized, requestIdPrefix);
    expect(nonce).to.exist;
    expect(nonce).to.equal(numberTo0x(mockData.account.ethereum_nonce));
  });
  ...
  it('should return latest nonce for latest block', async () => {
    restMock.onGet(accountPath).reply(200, mockData.account);
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, EthImpl.blockSafe, requestIdPrefix);
    expect(nonce).to.exist;
    expect(nonce).to.equal(numberTo0x(mockData.account.ethereum_nonce));
  });
  ...
  it('should return latest nonce for pending block', async () => {
    restMock.onGet(accountPath).reply(200, mockData.account);
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, EthImpl.blockPending, requestIdPrefix);
    expect(nonce).to.exist;
    expect(nonce).to.equal(numberTo0x(mockData.account.ethereum_nonce));
  });
  ...
  it('should return 0x0 nonce for earliest block with valid block', async () => {
    ...
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, EthImpl.blockEarliest, requestIdPrefix);
    expect(nonce).to.exist;
    expect(nonce).to.equal(EthImpl.zeroHex);
  });
  ...
  it('should return nonce for request on historical numerical block', async () => {
    ...
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, blockNumberHex, requestIdPrefix);
    expect(nonce).to.exist;
  ...
  it('should return valid nonce for historical numerical block when contract result sender is not address', async () => {
    ...
    const nonce = await ethImpl.getTransactionCount(mockData.account.evm_address, blockNumberHex, requestIdPrefix);
    expect(nonce).to.exist;
  ...
  it('should return nonce when block hash is passed', async () => {
    ...
    restMock
      .onGet(accountPathContractResultsAddress)
      .reply(200, { ...mockData.account, transactions: [defaultEthereumTransactions[0]] });
    const nonce = await ethImpl.getTransactionCount(MOCK_ACCOUNT_ADDR, blockHash, requestIdPrefix);
    expect(nonce).to.exist;
