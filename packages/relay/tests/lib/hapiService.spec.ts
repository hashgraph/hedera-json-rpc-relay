...
describe('HAPI Service', async function () {
  ...
  this.beforeEach(() => {
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_TRANSACTION_RESET', '0');
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_DURATION_RESET', '0');
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_ERROR_RESET', '[50]');
  });
  ...
  it('should be able to reinitialise SDK instance upon reaching transaction limit', async function () {
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_TRANSACTION_RESET', '2');
    hapiService = new HAPIService(logger, registry, hbarLimiter, cacheService, eventEmitter);
    expect(hapiService.getTransactionCount()).to.eq(
      parseInt(EnvProviderService.getInstance().get('HAPI_CLIENT_TRANSACTION_RESET')!),
    );
    ...
    expect(hapiService.getTransactionCount()).to.eq(
      parseInt(EnvProviderService.getInstance().get('HAPI_CLIENT_TRANSACTION_RESET')!) - 1,
  ...
  it('should be able to reinitialise SDK instance upon reaching time limit', async function () {
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_DURATION_RESET', '100');
    hapiService = new HAPIService(logger, registry, hbarLimiter, cacheService, eventEmitter);
    expect(hapiService.getTimeUntilReset()).to.eq(
      parseInt(EnvProviderService.getInstance().get('HAPI_CLIENT_DURATION_RESET')!),
    );
  ...
  it('should be able to reinitialise SDK instance upon error status code encounter', async function () {
    ...
    expect(hapiService.getErrorCodes()[0]).to.eq(
      JSON.parse(EnvProviderService.getInstance().get('HAPI_CLIENT_ERROR_RESET')!)[0],
    );
  ...
  it('should be able to reset all counter upon reinitialization of the SDK Client', async function () {
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_ERROR_RESET', '[50]');
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_TRANSACTION_RESET', '50');
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_DURATION_RESET', '36000');
    hapiService = new HAPIService(logger, registry, hbarLimiter, cacheService, eventEmitter);
  ...
  it('should not be able to reinitialise and decrement counters, if it is disabled', async function () {
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_TRANSACTION_RESET', '0');
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_DURATION_RESET', '0');
    EnvProviderService.getInstance().dynamicOverride('HAPI_CLIENT_ERROR_RESET', '[]');
