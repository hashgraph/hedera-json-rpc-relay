...
describe('ERC20 Acceptance Tests', function () {
    ...
    before(async () => {
        accounts[0] = await servicesNode.createAliasAccount(2000);
        contract = await servicesNode.deployContract(erc20Json, 2000000);
        // Wait for creation to propagate
        await mirrorNode.get(`/contracts/${contract.contractId}`);
    });

    it('should execute "eth_getCode" for ERC20 contract with evmAddress', async function () {
        const evmAddress = contract.contractId.toSolidityAddress();
        const res = await relay.call('eth_getCode', [evmAddress]);
        expect(res).to.eq(erc20Json.deployedBytecode);
    });
