...
describe('@erc20 Acceptance Tests', async function () {
    ...
    describe('HTS Transfer', function () {
        xit('should be able to execute transferFrom to another account', async function () {
            const amount = BigInt(10000);
            const tokenCreateContract = await Utils.createHTS(name, symbol, accounts[0], 10000, ERC20MockJson.abi, [accounts[1], accounts[2]], accounts[0], servicesNode, requestId);
            const IERC20 = await tokenCreateContract.attach(tokenCreateContract.target);

            const tokenCreateBalanceBefore = await IERC20.balanceOf(
              accounts[1].address
            );

            console.log('tokenCreateBalanceBefore', tokenCreateBalanceBefore);  
            const account0BalanceBefore = await IERC20.balanceOf(accounts[0].address);
            console.log('signer0BalanceBefore', account0BalanceBefore);
            const account1BalanceBefore = await IERC20.balanceOf(accounts[1].address);
            console.log('signer1BalanceBefore', account1BalanceBefore);  
        
            await IERC20.approve(accounts[1].address, amount, await Utils.gasOptions(requestId));
            const allowanceBefore = await IERC20.allowance(accounts[0].address, accounts[1].address);
            console.log('allowanceBefore', allowanceBefore);
        
            new ethers.Contract(accounts[1].address, ERC20MockJson.abi, accounts[1].wallet);
            const IERC20Signer1 = new ethers.Contract(accounts[1].address, ERC20MockJson.abi, accounts[1].wallet);
            await IERC20Signer1.transferFrom(
              accounts[0].address,
              accounts[1].address,
              amount,
              await Utils.gasOptions(requestId)
            );
        
            const allowanceAfter = await IERC20.allowance(accounts[0].address, accounts[1].address);
            ...
            console.log('tokenCreateBalanceAfter', tokenCreateBalanceAfter);
            const account0BalanceAfter = await IERC20.balanceOf(accounts[0].address);
            console.log('account0BalanceAfter', account0BalanceAfter);
            const account1BalanceAfter = await IERC20.balanceOf(accounts[1].address);
            ...
            expect(account0BalanceAfter).to.eq(account0BalanceBefore - amount);
            expect(account1BalanceAfter).to.eq(account1BalanceBefore);
          });        
    });
});
