...
describe('HTS Precompile Acceptance Tests', async function() {
  ...
  before(async () => {
    accounts[0] = await servicesNode.createAliasAccount(1000, relay.provider);
    accounts[1] = await servicesNode.createAliasAccount(1000, relay.provider);
    accounts[2] = await servicesNode.createAliasAccount(1000, relay.provider);
  });
