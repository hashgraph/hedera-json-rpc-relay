...
describe('@tokencreate HTS Precompile Token Create Acceptance Tests', async function () {
  ...
  it('should be able to mint a nft', async function () {
    ...
    const { serialNumbers } = (await tx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.MintedToken)[0].args;
  ...
  describe('HTS Precompile Approval Tests', async function () {
    ...
    it('should be able to approve anyone to spend tokens', async function () {
      ...
      const beforeAmount = (await txBefore.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.AllowanceValue)[0].args.amount;
      const { responseCode } = (await txBefore.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args;
      expect(responseCode).to.equal(TX_SUCCESS_CODE);
      ...
      // grant KYC
      {
        ...
        const responseCodeGrantKyc = (await grantKycTx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
        expect(responseCodeGrantKyc).to.equal(TX_SUCCESS_CODE);
      }
      {
        const grantKycTx = await mainContractOwner.grantTokenKycPublic(HTSTokenContractAddress, mainContract.target, Constants.GAS.LIMIT_1_000_000);
        const responseCodeGrantKyc = (await grantKycTx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
        expect(responseCodeGrantKyc).to.equal(TX_SUCCESS_CODE);
      }
      ...
      const responseCodeApproval = (await approvalTx.wait()).logs.filter(e => e?.fragment?.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
      ...
      const afterAmount = (await txAfter.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.AllowanceValue)[0].args.amount;
      ...
      await HTSTokenContract.connect(accounts[2].wallet).transferFrom(mainContract.target, accounts[1].wallet.address, amount, Constants.GAS.LIMIT_1_000_000);
...

      {
        ...
        const responseCodeRevokeKyc = (await revokeKycTx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
        expect(responseCodeRevokeKyc).to.equal(TX_SUCCESS_CODE);
      }
      {
        const revokeKycTx = await mainContractOwner.revokeTokenKycPublic(HTSTokenContractAddress, mainContract.target, Constants.GAS.LIMIT_1_000_000);
        const responseCodeRevokeKyc = (await revokeKycTx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
        expect(responseCodeRevokeKyc).to.equal(TX_SUCCESS_CODE);
      }
    });
    ...
    it('should be able to execute setApprovalForAllPublic', async function () {
      ...
      const beforeFlag = txBeforeReceipt.logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.Approved)[0].args.approved;
      const responseCodeTxBefore = txBeforeReceipt.logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
      ...
      const afterFlag = (await txAfter.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.Approved)[0].args.approved;
    ...
    it('should be able to execute getApproved on nft', async function () {
      ...
      const { approved } = (await tx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ApprovedAddress)[0].args;
    ...
    it('should be able to transfer nft with transferFrom', async function () {
      ...
      {
        const grantKycTx = await mainContractOwner.grantTokenKycPublic(NftHTSTokenContractAddress, mainContract.target, Constants.GAS.LIMIT_1_000_000);
        const responseCodeGrantKyc = (await grantKycTx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
        expect(responseCodeGrantKyc).to.equal(TX_SUCCESS_CODE);
      }
      ...
      await NFTokenContract.connect(accounts[2].wallet).transferFrom(mainContract.target, accounts[1].wallet.address, NftSerialNumber, Constants.GAS.LIMIT_1_000_000);
      await new Promise(r => setTimeout(r, 2000));
      ...
      {
        const revokeKycTx = await mainContractOwner.revokeTokenKycPublic(NftHTSTokenContractAddress, mainContract.target, Constants.GAS.LIMIT_1_000_000);
        const responseCodeRevokeKyc = (await revokeKycTx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
        expect(responseCodeRevokeKyc).to.equal(TX_SUCCESS_CODE);
      }
    });
  ...
  describe('HTS Precompile Get Token Info Tests', async function () {
    it('should be able to get fungible token info', async () => {
      ...
      const { tokenInfo, decimals } = (await tx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.FungibleTokenInfo)[0].args.tokenInfo;
    ...
    it('should be able to get token info', async () => {
      ...
      const { token, totalSupply } = (await tx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.TokenInfo)[0].args.tokenInfo;
    ...
    it('should be able to get non-fungible token info', async () => {
      ...
      const { tokenInfo, serialNumber } = (await tx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.NonFungibleTokenInfo)[0].args.tokenInfo;
  ...
  describe('HTS Precompile KYC Tests', async function () {
    async function checkKyc(contractOwner, tokenAddress, accountAddress, expectedValue: boolean) {
      ...
      const responseCodeIsKyc = (await tx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
      expect(responseCodeIsKyc).to.equal(TX_SUCCESS_CODE);

      const isKycGranted = (await tx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.KycGranted)[0].args.kycGranted;
    ...
    async function checkTokenDefaultKYCStatus(contractOwner, tokenAddress, expectedValue: boolean) {
      ...
      const responseCodeTokenDefaultStatus = (await txTokenDefaultStatus.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
      const defaultTokenKYCStatus = (await txTokenDefaultStatus.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.TokenDefaultKycStatus)[0].args.defaultKycStatus;
  ...
  describe('HTS Precompile Custom Fees Tests', async function () {
    it('should be able to get a custom token fees', async function () {
      ...
      const { fixedFees, fractionalFees } = (await tx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.TokenCustomFees)[0].args;
  ...
  describe('HTS Precompile Delete Token Tests', async function () {
    it('should be able to delete a token', async function () {
      ...
      const tokenInfoBefore = (await txBefore.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.TokenInfo)[0].args.tokenInfo;

      const tx = await mainContract.deleteTokenPublic(createdTokenAddress, Constants.GAS.LIMIT_1_000_000);
      const responseCode = (await tx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode;
      expect(responseCode).to.equal(TX_SUCCESS_CODE);

      const txAfter = (await mainContract.getTokenInfoPublic(createdTokenAddress, Constants.GAS.LIMIT_1_000_000));
      const tokenInfoAfter = (await txAfter.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.TokenInfo)[0].args.tokenInfo;
  ...
  describe('CryptoTransfer Tests', async function () {
    ...
    async function setKyc(tokenAddress) {
      ...
      expect((await grantKycTx.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode).to.equal(TX_SUCCESS_CODE);
      ...
      expect((await grantKycTx1.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode).to.equal(TX_SUCCESS_CODE);
      ...
      expect((await grantKycTx2.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode).to.equal(TX_SUCCESS_CODE);
    }
    ...
    it('should be able to transfer non-fungible tokens', async function () {
      ...
      expect((await txMint.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.ResponseCode)[0].args.responseCode).to.be.equal(TX_SUCCESS_CODE);
      const { serialNumbers } = (await txMint.wait()).logs.filter(e => e.fragment.name === Constants.HTS_CONTRACT_EVENTS.MintedToken)[0].args;
      NftSerialNumber = Number(serialNumbers[0]);
