    ...
    describe('RPC Server Acceptance Tests', function () {
        ...
        describe('eth_getLogs', () => {
            ...
            it('should be able to use range of `fromBlock` and `toBlock` params', async () => {
                // Logs are currently ordered in desc order, which is not correct. The tests should be fixed once the issue is fixed.
                const logs = await relay.call('eth_getLogs', [{
                    'fromBlock': log0Block.blockNumber,
                    'toBlock': log4Block.blockNumber
                }], requestId);
                ...
                for (const i in logs) {
                    expect(parseInt(logs[i].blockNumber, 16)).to.be.greaterThanOrEqual(parseInt(log0Block.blockNumber));
                    expect(parseInt(logs[i].blockNumber, 16)).to.be.lessThanOrEqual(parseInt(log4Block.blockNumber));
                }
            });
            ...
            it('should be able to use `topics` param', async () => {
                ...
                expect(logs.length).to.be.greaterThan(0);
                const topic = logs[1].topics[0]; // log[0] is anonymous and topic0 is not the event signature, but would potentially be the first indexed param if it has any
        ...
        describe('Block related RPC calls', () => {
            ...
            it('@release should execute "eth_estimateGas"', async function () {
                const res = await relay.call('eth_estimateGas', [{"to": '0x' + accounts[0].address}, 'latest'], requestId);
            ...
            it('@release should fail "eth_getBalance" with block number in the last 15 minutes', async function () {
                ...
                try {
                    await relay.call('eth_getBalance', [Utils.idToEvmAddress(contractId.toString()), EthImpl.numberTo0x(earlierBlockNumber)], requestId);
                }
                catch(error) {
        ...
        if (process.env.LOCAL_NODE && process.env.LOCAL_NODE !== 'false') {
            describe('Gas Price related RPC endpoints', () => {
                let lastBlockBeforeUpdate: any;
                let lastBlockAfterUpdate: any;
