    ...
    describe('RPC Server Acceptance Tests', function () {
        ...
        describe('eth_getLogs', () => {
            ...
            before(async () => {
                ...
                await new Promise(r => setTimeout(r, 5000));
                latestBlock = Number(await relay.call(RelayCalls.ETH_BLOCK_NUMBER, [], requestId));
            ...
            it('@release should deploy a contract', async () => {
                ...
                const logs = await relay.call(RelayCalls.ETH_GET_LOGS, [{
                ...
                log0Block = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_HASH, [logs[0].transactionHash], requestId);
                expect(log0Block).to.have.property('blockNumber');
                expect(log0Block.nonce).to.equal('0x0');

                log4Block = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_HASH, [logs[4].transactionHash], requestId);
            ...
            it('should not be able to use `toBlock` without `fromBlock` param if `toBlock` is not latest', async () => {
                await relay.callFailing(RelayCalls.ETH_GET_LOGS, [{
            ...
            it('should be able to use `address` param with multiple addresses', async () => {
                const logs = await relay.call(RelayCalls.ETH_GET_LOGS, [{
                    'fromBlock': EthImpl.numberTo0x(tenBlocksBehindLatest),
                    'address': [contractAddress, contractAddress2, Constants.NON_EXISTING_ADDRESS]
                }], requestId);
                expect(logs.length).to.be.greaterThan(0);
            ...
            it('should return empty result for  non-existing `blockHash`', async () => {
                const logs = await relay.call(RelayCalls.ETH_GET_LOGS, [{
                    'blockHash': Constants.NON_EXISTING_BLOCK_HASH
                }], requestId);
            ...
            it('should be able to use `topics` param', async () => {
                ...
                const logsWithTopic = await relay.call(RelayCalls.ETH_GET_LOGS, [{
                    'fromBlock': log0Block.blockNumber,
                    'toBlock': log4Block.blockNumber,
                    'topics': [topic]
                }], requestId);
            ...
            it('should be able to return more than 2 logs with limit of 2 logs per request', async () => {
                ...
                const currentBlock = Number(await relay.call(RelayCalls.ETH_BLOCK_NUMBER, [], requestId));
        ...
        describe('Block related RPC calls', () => {
            ...
            it('should execute "eth_getBlockByHash", hydrated transactions = false', async function () {
                const blockResult = await relay.call(RelayCalls.ETH_GET_BLOCK_BY_HASH, [mirrorBlock.hash.substring(0, 66), false], requestId);
            ...
            it('@release should execute "eth_getBlockByHash", hydrated transactions = true', async function () {
                const blockResult = await relay.call(RelayCalls.ETH_GET_BLOCK_BY_HASH, [mirrorBlock.hash.substring(0, 66), true], requestId);
            ...
            it('should execute "eth_getBlockByHash" for non-existing block hash and hydrated transactions = false', async function () {
                const blockResult = await relay.call(RelayCalls.ETH_GET_BLOCK_BY_HASH, [Constants.NON_EXISTING_BLOCK_HASH, false], requestId);
            ...
            it('should execute "eth_getBlockByHash" for non-existing block hash and hydrated transactions = true', async function () {
                const blockResult = await relay.call(RelayCalls.ETH_GET_BLOCK_BY_HASH, [Constants.NON_EXISTING_BLOCK_HASH, true], requestId);
                expect(blockResult).to.be.null;
            });

            it('should execute "eth_getBlockByNumber", hydrated transactions = false', async function () {
                const blockResult = await relay.call(RelayCalls.ETH_GET_BLOCK_BY_NUMBER, [EthImpl.numberTo0x(mirrorBlock.number), false], requestId);
                Assertions.block(blockResult, mirrorBlock, mirrorTransactions, false);
            });

            it('@release should execute "eth_getBlockByNumber", hydrated transactions = true', async function () {
                const blockResult = await relay.call(RelayCalls.ETH_GET_BLOCK_BY_NUMBER, [EthImpl.numberTo0x(mirrorBlock.number), true], requestId);
                Assertions.block(blockResult, mirrorBlock, mirrorTransactions, true);
            });

            it('should execute "eth_getBlockByNumber" for non existing block number and hydrated transactions = true', async function () {
                const blockResult = await relay.call(RelayCalls.ETH_GET_BLOCK_BY_NUMBER, [Constants.NON_EXISTING_BLOCK_NUMBER, true], requestId);
                expect(blockResult).to.be.null;
            });

            it('should execute "eth_getBlockByNumber" for non existing block number and hydrated transactions = false', async function () {
                const blockResult = await relay.call(RelayCalls.ETH_GET_BLOCK_BY_NUMBER, [Constants.NON_EXISTING_BLOCK_NUMBER, false], requestId);
                expect(blockResult).to.be.null;
            });

            it('@release should execute "eth_getBlockTransactionCountByNumber"', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_BLOCK_TRANSACTION_COUNT_BY_NUMBER, [EthImpl.numberTo0x(mirrorBlock.number)], requestId);
            ...
            it('should execute "eth_getBlockTransactionCountByNumber" for non-existing block number', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_BLOCK_TRANSACTION_COUNT_BY_NUMBER, [Constants.NON_EXISTING_BLOCK_NUMBER], requestId);
                expect(res).to.be.null;
            });

            it('@release should execute "eth_getBlockTransactionCountByHash"', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_BLOCK_TRANSACTION_COUNT_BY_HASH, [mirrorBlock.hash.substring(0, 66)], requestId);
                expect(res).to.be.equal(ethers.utils.hexValue(mirrorBlock.count));
            });

            it('should execute "eth_getBlockTransactionCountByHash" for non-existing block hash', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_BLOCK_TRANSACTION_COUNT_BY_HASH, [Constants.NON_EXISTING_BLOCK_HASH], requestId);
                expect(res).to.be.null;
            });
            ...
            it('should execute "eth_getBlockTransactionCountByNumber"', async function () {
                ...
                it('@release should execute "eth_blockNumber"', async function () {
                    ...
                    const res = await relay.call(RelayCalls.ETH_BLOCK_NUMBER, [], requestId);
        ...
        describe('Transaction related RPC Calls', () => {
            ...
            it('@release should execute "eth_getTransactionByBlockHashAndIndex"', async function () {
                const response = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_BLOCK_HASH_AND_INDEX,
            ...
            it('should execute "eth_getTransactionByBlockHashAndIndex" for invalid block hash', async function () {
                const response = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_BLOCK_HASH_AND_INDEX,
                    [Constants.NON_EXISTING_BLOCK_HASH, EthImpl.numberTo0x(mirrorContractDetails.transaction_index)], requestId);
                expect(response).to.be.null;
            });

            it('should execute "eth_getTransactionByBlockHashAndIndex" for invalid index', async function () {
                const response = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_BLOCK_HASH_AND_INDEX,
                    [mirrorContractDetails.block_hash.substring(0, 66), Constants.NON_EXISTING_INDEX], requestId);
                expect(response).to.be.null;
            });

            it('@release should execute "eth_getTransactionByBlockNumberAndIndex"', async function () {
                const response = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_BLOCK_NUMBER_AND_INDEX, [EthImpl.numberTo0x(mirrorContractDetails.block_number), EthImpl.numberTo0x(mirrorContractDetails.transaction_index)], requestId);
                Assertions.transaction(response, mirrorContractDetails);
            });

            it('should execute "eth_getTransactionByBlockNumberAndIndex" for invalid index', async function () {
                const response = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_BLOCK_NUMBER_AND_INDEX, [EthImpl.numberTo0x(mirrorContractDetails.block_number), Constants.NON_EXISTING_INDEX], requestId);
                expect(response).to.be.null;
            });

            it('should execute "eth_getTransactionByBlockNumberAndIndex" for non-exising block number', async function () {
                const response = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_BLOCK_NUMBER_AND_INDEX, [Constants.NON_EXISTING_BLOCK_NUMBER, EthImpl.numberTo0x(mirrorContractDetails.transaction_index)], requestId);
                expect(response).to.be.null;
            });
            ...
            it('@release should execute "eth_getTransactionReceipt" for hash of legacy transaction', async function () {
                ...
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_RECEIPT, [legacyTxHash], requestId);
            ...
            it('@release should execute "eth_getTransactionReceipt" for hash of London transaction', async function () {
                ...
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_RECEIPT, [transactionHash], requestId);
                // FIXME here we must assert that the alias address is the `from` / `to` and not the `0x` prefixed one
                Assertions.transactionReceipt(res, mirrorResult);
            });
            ...
            it('should execute "eth_getTransactionReceipt" for non-existing hash', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_RECEIPT, [Constants.NON_EXISTING_TX_HASH], requestId);
                expect(res).to.be.null;
            });
            ...
            it('should fail "eth_sendRawTransaction" for legacy EIP 155 transactions (with insufficient balance)', async function () {
                ...
                const signedTx = await accounts[2].wallet.signTransaction(transaction);
                await relay.callFailing(RelayCalls.ETH_SEND_RAW_TRANSACTION, [signedTx], predefined.INSUFFICIENT_ACCOUNT_BALANCE, requestId);
            });
            ...
            it('should fail "eth_sendRawTransaction" for Legacy transactions (with no chainId)', async function () {
                ...
                const signedTx = await accounts[2].wallet.signTransaction(transaction);
                await relay.callFailing(RelayCalls.ETH_SEND_RAW_TRANSACTION, [signedTx], predefined.UNSUPPORTED_CHAIN_ID('0x0', CHAIN_ID), requestId);
            });
            ...
            it('should fail "eth_sendRawTransaction" for Legacy transactions (with gas price too low)', async function () {
                ...
                const signedTx = await accounts[2].wallet.signTransaction(transaction);
                await relay.callFailing(RelayCalls.ETH_SEND_RAW_TRANSACTION, [signedTx], predefined.GAS_PRICE_TOO_LOW, requestId);
            });
            ...
            it('should execute "eth_sendRawTransaction" for London transactions', async function () {
                ...
                const signedTx = await accounts[2].wallet.signTransaction(transaction);
                const transactionHash = await relay.call(RelayCalls.ETH_SEND_RAW_TRANSACTION, [signedTx], requestId);
            ...
            it('@release should execute "eth_getTransactionCount" primary', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_COUNT, [mirrorPrimaryAccount.evm_address, EthImpl.numberTo0x(mirrorContractDetails.block_number)], requestId);
                expect(res).to.be.equal('0x0');
            });

            it('should execute "eth_getTransactionCount" secondary', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_COUNT, [mirrorSecondaryAccount.evm_address, EthImpl.numberTo0x(mirrorContractDetails.block_number)], requestId);
                expect(res).to.be.equal('0x0');
            });

            it('@release should execute "eth_getTransactionCount" contract', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_COUNT, [mirrorContract.evm_address, EthImpl.numberTo0x(mirrorContractDetails.block_number)], requestId);
            ...
            it('@release should execute "eth_getTransactionCount" for account with id converted to evm_address', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_COUNT, [Utils.idToEvmAddress(mirrorPrimaryAccount.account), EthImpl.numberTo0x(mirrorContractDetails.block_number)], requestId);
                expect(res).to.be.equal('0x0');
            });

            it('@release should execute "eth_getTransactionCount" contract with id converted to evm_address', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_COUNT, [Utils.idToEvmAddress(contractId.toString()), EthImpl.numberTo0x(mirrorContractDetails.block_number)], requestId);
                expect(res).to.be.equal('0x1');
            });

            it('should execute "eth_getTransactionCount" for non-existing address', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_COUNT, [Constants.NON_EXISTING_ADDRESS, EthImpl.numberTo0x(mirrorContractDetails.block_number)], requestId);
                expect(res).to.be.equal('0x0');
            });
            ...
            it('should execute "eth_getTransactionCount" from hollow account', async function() {
                ...
                const resBeforeCreation = await relay.call(RelayCalls.ETH_GET_TRANSACTION_COUNT, [hollowAccount.address, 'latest'], requestId);
                ...
                const txHashHAC = await relay.call(RelayCalls.ETH_SEND_RAW_TRANSACTION, [signedTxHollowAccountCreation], requestId);
                ...
                const txHashHA = await relay.call(RelayCalls.ETH_SEND_RAW_TRANSACTION, [signTxFromHollowAccount], requestId);
                await mirrorNode.get(`/contracts/results/${txHashHA}`, requestId);

                const resAfterCreation = await relay.call(RelayCalls.ETH_GET_TRANSACTION_COUNT, [hollowAccount.address, 'latest'], requestId);
            ...
            it('should execute "eth_getTransactionCount" for account with non-zero nonce', async function () {
                ...
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_COUNT, ['0x' + account.address, 'latest'], requestId);
                expect(res).to.be.equal('0x1');
            });
            ...
            it('@release should execute "eth_getTransactionByHash" for existing transaction', async function () {
                ...
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_HASH, [transactionHash], requestId);
            ...
            it('should execute "eth_getTransactionByHash" for non-existing transaction and return null', async function () {
                const res = await relay.call(RelayCalls.ETH_GET_TRANSACTION_BY_HASH, [Constants.NON_EXISTING_TX_HASH], requestId);
                expect(res).to.be.null;
            });
        });
    });
