...
describe('@api-batch-1 RPC Server Acceptance Tests', function () {
  ...
  describe('RPC Server Acceptance Tests', function () {
    ...
    describe('Transaction related RPC Calls', () => {
      ...
      it.only('should execute "eth_getTransactionReceipt" for a transfer transaction', async function () {
        ...
        const mirrorResult = await mirrorNode.get(`/contracts/results/${tx.hash}`, requestId);
        Assertions.transaction(txByHash, mirrorResult);
        Assertions.transactionReceipt(rec, mirrorResult);
      });
