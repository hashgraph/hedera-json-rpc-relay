...
export default class ConnectionLimiter {
  ...
  constructor(logger: Logger, register: Registry) {
    ...
    const rateLimitDuration = EnvProvider.get('LIMIT_DURATION')
      ? // @ts-ignore
        parseInt(EnvProvider.get('LIMIT_DURATION'))
  ...
  public applyLimits(ctx) {
    ...
    const MAX_CONNECTION_LIMIT = EnvProvider.get('WS_CONNECTION_LIMIT') || '10';
    ...
    const { ip } = ctx.request;
    const MAX_CONNECTION_LIMIT_PER_IP = EnvProvider.get('WS_CONNECTION_LIMIT_PER_IP') || '10';
  ...
  public validateSubscriptionLimit(ctx) {
    return ctx.websocket.subscriptions < parseInt(EnvProvider.get('WS_SUBSCRIPTION_LIMIT') || '10');
  }

  // Starts a timeout timer that closes the connection
  public startInactivityTTLTimer(websocket) {
    const maxInactivityTTL = parseInt(EnvProvider.get('WS_MAX_INACTIVITY_TTL') || '300000');
