...
module.exports = async () => {
  ...
  const ft = await hre.viem.deployContract('FungibleToken', [100]);

  console.log(`FungibleToken deployed to: ${ft.address}`);

  const nft = await hre.viem.deployContract('NonFungibleToken', []);

  console.log(`FungibleToken deployed to: ${nft.address}`);
